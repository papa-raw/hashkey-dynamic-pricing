...
export function Sidebar() {
  ...
  return (
    <aside className="w-[220px] flex-shrink-0 border-r border-pp-border-sub bg-pp-bg flex flex-col h-screen sticky top-0">
      ...
      {showInfo && (
        <>
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50" onClick={() => setShowInfo(false)} />
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" onClick={() => setShowInfo(false)}>
          <div className="bg-[#141C26] border border-pp-border rounded-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto p-6 text-xs text-pp-secondary space-y-4 shadow-2xl" onClick={e => e.stopPropagation()}>
